...
#                 truncation=True,
#                 padding=True,
#                 max_length=512  # ✅ added to suppress warning and improve performance
#             )
...
#             with torch.no_grad():
#                 outputs = self.model(**inputs)
#                 scores = outputs.logits.squeeze().softmax(dim=0)
#                 prediction = scores.argmax().item()

#             sentiment_category = self.labels[prediction]
#             sentiment_score = scores[prediction].item()
...
#     def analyze_social_media_data(self, data):
#         """Custom analyzer for your data format"""
#         if not data:
#             return []
...
#             try:
#                 # Get text from either cleaned or original
#                 text = item.get('cleaned_text') or item.get('original_text') or ''
...
#                 # Predict sentiment
#                 sentiment_data = self._predict_sentiment(text)
...
#                     'metrics': {
#                         'likes': item.get('tweet_like_count') or item.get('likes_count'),
#                         'shares': item.get('tweet_retweet_count')
#                     }
#                 }
...
#             except Exception as e:
#                 print(f"Error analyzing item {item.get('id')}: {str(e)}")
#                 continue
