...
async function loadPowerBIDashboard() {
    ...
    if (!powerbiFrame) return;

    try {
        const response = await fetch(`${SERVER_URL}/get-powerbi-token`, {
            method: "POST",
            headers: { "Content-Type": "application/json" }
        });
        ...
        if (result.embedToken && result.embedUrl) {
            powerbiFrame.innerHTML = `
                <iframe
                    width="100%"
                    height="600"
                    src="${result.embedUrl}&embedToken=${result.embedToken}"
                    frameborder="0"
                    allowFullScreen="true">
                </iframe>
            `;
        } else {
            powerbiFrame.innerHTML = `<p>❌ Failed to load Power BI report</p>`;
        }
    } catch (err) {
        powerbiFrame.innerHTML = `<p>🚫 Error loading dashboard: ${err.message}</p>`;
    }
}
...
function refreshPowerBI() {
    const powerbiFrame = document.getElementById("powerbi-frame");
    if (powerbiFrame) {
        powerbiFrame.src = powerbiFrame.src; // Reloads the iframe
    }
}

// 🕒 Optional: Auto-refresh every 5 minutes (ONLY if tab is open)
setInterval(() => {
    const dashboardPane = document.getElementById("dashboard");
    const powerbiFrame = document.getElementById("powerbi-frame");
    if (dashboardPane && dashboardPane.classList.contains("active") && powerbiFrame) {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (dashboardTab) {
        dashboardTab.addEventListener("click", loadPowerBIDashboard);
    }
});
...
async function importData(clearBeforeImport = false) {
    try {
        ...
        // Auto-run COMPLETE analysis (replace all the custom logic below)
        setTimeout(async () => {
            updateDataCounts();
            const total = appState.twitterData.length + appState.instagramData.length;
            
            if (total > 0) {
                console.log('Starting automatic complete analysis after import...');
                // Simply call the existing runAnalysis function instead of custom logic
                await runAnalysis();
                console.log('Automatic analysis completed after import');
            }
        }, 500);
